...
| <a name="input_spot_allocation_strategy"></a> [spot\_allocation\_strategy](#input\_spot\_allocation\_strategy) | Strategy to use when launching Spot instances. Valid values: lowest-price, capacity-optimized, capacity-optimized-prioritized, price-capacity-optimized | `string` | `"price-capacity-optimized"` | no |
| <a name="input_spot_instance_pools"></a> [spot\_instance\_pools](#input\_spot\_instance\_pools) | Number of Spot pools per availability zone to allocate capacity. Active only when spot\_allocation\_strategy is set to lowest-price | `number` | `null` | no |
